 ...
 * Choose between Interview Prep or Office English
 */
...
export default function ModeSelectPage() {
  ...
  const modes = [
    ...
    },
    {
      ...
      description: 'One focused scenario per day. 2-3 minutes. Practice a specific interview skill and build your daily speaking habit.',
  ...
  return (
    <div className="min-h-screen flex flex-col px-6 py-8">
      ...
      <div className="flex-1 flex flex-col gap-4 max-w-md mx-auto w-full">
        {modes.map((mode, index) => (
          <ModeCard
            ...
            icon={mode.icon}
            color={mode.color}
            onClick={() => router.push(mode.id === 'interview' ? '/interview-setup' : '/daily-challenge')}
            delay={`${index * 0.1}s`}
          />
        ))}
      </div>
      ...
      <p className="text-center text-sm text-gray-400 mt-8 animate-fade-in" style={{ animationDelay: '0.4s' }}>
        Tip: Practice daily to build real confidence
      </p>
    </div>
  );
}
